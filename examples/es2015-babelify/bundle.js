...
_srcLoaderModule2['default'].track('Tracking after alooma.init');

},{"../../src/loader-module":3}],2:[function(require,module,exports){
/*
 * Alooma JS Library
 *
 * Copyright 2012, Alooma, Inc. All Rights Reserved
 * http://alooma.com/
 *
...
// @output_file_name alooma-2.7.min.js
// ==/ClosureCompiler==
 ...
 * @constructor
 */
var AloomaPersistence = function AloomaPersistence(config) {
    this['props'] = {};
...
AloomaPersistence.prototype.set_event_timer = function (event_name, timestamp) {
...
AloomaPersistence.prototype.remove_event_timer = function (event_name) {
    var timers = this['props'][EVENT_TIMERS_KEY] || {};
...
var create_mplib = function create_mplib(token, config, name) {
    var instance,
        target = name === PRIMARY_INSTANCE_NAME ? alooma_master : alooma_master[name];
 ...
 * @constructor
 */
var AloomaLib = function AloomaLib() {};
...
AloomaLib.prototype.track = function (event_name, properties, callback) {
    ...
    properties = _.extend({}, _.info.properties(), this['persistence'].properties(), this['__request_properties'], properties);
 ...
 */
AloomaLib.prototype.time_event = function (event_name) {
    if (_.isUndefined(event_name)) {
        console.error("No event name provided to alooma.time_event");
        return;
    }
...
AloomaLib.prototype.register_request = function (props) {
    this['__request_properties'] = _.extend(this['__request_properties'], props);
};
...
AloomaLib.prototype._event_is_disabled = function (event_name) {
 ...
 * @constructor
 */
var AloomaPeople = function AloomaPeople() {};

AloomaPeople.prototype._init = function (alooma_instance) {
    this._alooma = alooma_instance;
};
...
AloomaPeople.prototype.set = function (prop, to, callback) {
    ...
    $set = _.extend({}, _.info.people_properties(), this._alooma['persistence'].get_referrer_info(), $set);
...
AloomaPeople.prototype._send_request = function (data, callback) {
    ...
    this._alooma._send_request(this._get_config('api_host') + '/engage/', { 'data': encoded_data }, this._alooma._prepare_callback(callback, truncated_data));
...
var override_mp_init_func = function override_mp_init_func() {
    ...
    alooma_master['init'] = function (token, config, name) {
        if (name) {
            // initialize a sub library
            if (!alooma_master[name]) {
                alooma_master[name] = instances[name] = create_mplib(token, config, name);
                alooma_master[name]._loaded();
            }
            return alooma_master[name];
        } else {
            var instance = alooma_master;
            ...
            alooma_master = instance;
            if (init_type === INIT_SNIPPET) {
                window[PRIMARY_INSTANCE_NAME] = alooma_master;
            }
            extend_mp();
        }
    };
...
function init_from_snippet() {
    ...
    alooma_master = window[PRIMARY_INSTANCE_NAME];

    // Initialization
    if (_.isUndefined(alooma_master)) {
    ...
    if (alooma_master['__loaded'] || alooma_master['config'] && alooma_master['persistence']) {
        // lib has already been loaded at least once; we don't want to override the global object this time so bomb early
        console.error("Alooma library has already been downloaded at least once.");
        return;
    }
    var snippet_version = alooma_master['__SV'] || 0;
    ...
    _.each(alooma_master['_i'], function (item) {
...
function init_as_module() {
    ...
    alooma_master = new AloomaLib();

    override_mp_init_func();
    alooma_master['init']();
    add_dom_loaded_handler();

    return alooma_master;
}
...
var _aloomaCore = require('./alooma-core');

var alooma = (0, _aloomaCore.init_as_module)();

exports['default'] = alooma;
