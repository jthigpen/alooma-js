...
mpmodule("alooma.track");
    ...
    asyncTest("check callback", 1, function() {
        alooma.test.track('test', {}, function(response) {
    ...
    asyncTest("check no property name aliasing occurs during minify", 1, function() {
        ...
        alooma.test.track('test', ob, function(response) {
    ...
    test("token property does not override configured token", 1, function() {
        ...
        var data = alooma.test.track('test', props);
        same(data.properties.token, alooma.test.get_config('token'), 'Property did not override token');
    });
    ...
    test("disable() disables all tracking from firing", 2, function() {
        ...
        alooma.test.track("event_a", {}, function(response) {
            same(response, 0, "track should return an error");
            start();
        });

        alooma.test.track("event_b", {}, function(response) {
            same(response, 0, "track should return an error");
            start();
        });
    });
    ...
    test("disable([event_arr]) disables individual events", 3, function() {
        ...
        alooma.test.disable(['event_a']);
        alooma.test.disable(['event_c']);
        ...
        alooma.test.track("event_c", {}, function(response) {
            same(response, 0, "track should return an error");
            start();
        });
    });
    ...
    test("img based tracking", function() {
        ...
        if (window.console) {
            ...
            callsError(function(restore_console) {
                alooma.test.track_links(e1.class_name, "link_clicked");
                restore_console();
                start();
...
if (!window.COOKIE_FAILURE_TEST) {
    mpmodule("cookies");
        ...
        test("cookie name", 6, function() {
            ...
            clearLibInstance(alooma.cn1);
            clearLibInstance(alooma.cn2);
        });

        test("cross subdomain", 4, function() {
            var name = alooma.test.config.cookie_name;

            ok(alooma.test.cookie.get_cross_subdomain(), "Cross subdomain should be set correctly");
            // Remove non-cross-subdomain cookie if it exists.
            cookie.remove(name, false);
            ok(cookie.exists(name), "Cookie should still exist");

            alooma.test.set_config({ cross_subdomain_cookie: false });
            notOk(alooma.test.cookie.get_cross_subdomain(), "Should switch to false");
        ...
        asyncTest("secure cookie false by default", 1, function() {
            cookie_included(alooma.test.cookie.name, function(resp) {
        ...
        asyncTest("secure cookie only sent to https", 1, function() {
            alooma.test.set_config({ secure_cookie: true });
...
mpmodule("alooma");
    ...
    test("initial referrers set correctly", 8, function() {
        ...
        alooma.test.track_pageview();

        ok(i_ref in alooma.test.persistence.props, "initial referrer saved");
        ok(i_ref_d in alooma.test.persistence.props, "initial referring domain saved");
        ...
        equal(alooma.test.persistence.props[i_ref_d], none_val, "$initial_referring_domain should remain None even after getting a referrer");
...
mpmodule("super properties");
    ...
    test("register", 2, function() {
        var props = {'hi': 'there'},
            persisted_props = get_props_without_distinct_id(alooma.test);

        same(persisted_props, {}, "empty before setting");

        alooma.test.register(props);

        same(get_props_without_distinct_id(alooma.test), props, "properties set properly");
    });
    ...
    test("register_once", 3, function() {
        ...
        same(get_props_without_distinct_id(alooma.test), {}, "empty before setting");

        alooma.test.register_once(props);

        same(get_props_without_distinct_id(alooma.test), props, "properties set properly");

        alooma.test.register_once(props1);

        same(get_props_without_distinct_id(alooma.test), props, "register_once doesn't override already set super property");
    });

    test("identify", 1, function() {
        alooma.test.identify(this.id);
        same(alooma.test.get_distinct_id(), this.id);
    });
    ...
    test("name_tag", 2, function() {
        ...
        alooma.test.name_tag(name_tag);
        same(get_props_without_distinct_id(alooma.test), { 'mp_name_tag': name_tag }, "name tag set");
    });
    ...
    test("super properties overridden by manual props", 2, function() {
        ...
        var data = alooma.test.track('test', {'a': 'c'});
        var dp = data.properties;
...
mpmodule("alooma.alias");
    ...
    test("alias sends an event", 2, function() {
        var old_id = alooma.test.get_distinct_id(),
            new_id = this.id;

        var ev = alooma.test.alias(new_id);
    ...
    test("continues to use old ID after alias call", 3, function() {
        ...
        alooma.test.alias(new_id);
        same(alooma.test.get_distinct_id(), old_id);
        same(alooma.test.get_property(__alias), new_id);
    });

    test("aliasing same ID returns error code", 1, function() {
        var old_id = alooma.test.get_distinct_id(),
            ev = alooma.test.alias(old_id);
    ...
    test("alias prevents identify from changing the ID", 3, function() {
        ...
        notOk(old_id === new_id);
        alooma.test.alias(new_id);
        alooma.test.identify(new_id);
        same(alooma.test.get_distinct_id(), old_id, "identify should not do anything");
        same(alooma.test.get_property(__alias), new_id, "identify should not delete the __alias key");
    });
    ...
    test("identify with completely new ID blows away alias", 3, function() {
        ...
        alooma.test.alias(alias);
        alooma.test.identify(new_id);
        same(alooma.test.get_distinct_id(), new_id, "identify should replace the distinct id");
        same(alooma.test.get_property(__alias), undefined, "__alias should get blown away");
    });
    ...
    test("alias not in props", 3, function() {
        ...
        alooma.test.alias(new_id);
        same(alooma.test.get_property(__alias), new_id, "identify should not delete the __alias key");
        notOk(__alias in alooma.test.persistence.properties())
    });

    test("alias not allowed when there is previous people distinct id", 2, function() {
        alooma.test.register({"$people_distinct_id": this.id});
        same(alooma.test.alias(this.id), -2);
        same(alooma.test.get_property(__alias), undefined, "__alias should not be set");
    });
...
mpmodule("alooma.people.set");
    ...
    test("set queues data", 2, function() {
        stop();
        s = alooma.test.people.set({ a: 2 }, function(resp) {
            same(resp, -1, "responded with 'queued'");
            start();
        });
        ok(contains_obj(alooma.test.persistence.props['__mps'], { a: 2 }), "queued set saved");
    });
    ...
    test("set hits server immediately if identified", 4, function() {
        ...
        stop();
        s = alooma.test.people.set({ a: 3 }, function(resp) {
    ...
    test("set (info props included)", 5, function() {
        ...
        var data = alooma.people.set('key1', 'test');
...
mpmodule("alooma.people.set_once");
    ...
    test("set_once queues data", 2, function() {
        stop();
        s = alooma.test.people.set_once({ a: 2 }, function(resp) {
            same(resp, -1, "responded with 'queued'");
            start();
        });
        ok(contains_obj(alooma.test.persistence.props['__mpso'], { a: 2 }), "queued set_once saved");
    });
    ...
    test("set_once hits server immediately if identified", 4, function() {
        ...
        stop();
        s = alooma.test.people.set_once({ a: 3 }, function(resp) {
            same(resp, 1, "responded with 'success'");
            start();
        });
    ...
    test("set_once (info props not included)", 5, function() {
        ...
        var data = alooma.people.set_once('key1', 'test');
    ...
    test("queued set_once calls don't override previously queued calls", 3, function() {
        ...
        s = alooma.test.people.set_once({ a: 3, b: 4 }, function(resp) {
            same(resp, -1, "responded with 'queued'");
        });
        ok(contains_obj(alooma.test.persistence.props['__mpso'], { a: 2, b: 4 }), "queued set_once call works correctly");
    });

mpmodule("alooma.people.increment");
    ...
    test("increment queues data", 2, function() {
        stop();
        s = alooma.test.people.increment({ a: 2 }, function(resp) {
            same(resp, -1, "responded with 'queued'");
            start();
        });
        same(alooma.test.persistence.props['__mpa'], { a: 2 }, "queued increment saved");
    });
    ...
    test("increment hits server immediately if identified", 2, function() {
        ...
        stop();
        s = alooma.test.people.increment({ a: 3 }, function(resp) {
            same(resp, 1, "responded with 'success'");
            start();
        });
    ...
    test("increment ignores string values", 1, function() {
        alooma.test.identify(this.id);
        i = alooma.test.people.increment({ "a" : 1, "name": "joe" });
        same(i, { "$distinct_id": this.id, "$token": this.token, "$add": { "a": 1 }}, "string value ignored");
    });

mpmodule("alooma.people.append");
    ...
    test("append queues data", 2, function() {
        stop();
        s = alooma.test.people.append({ a: 2 }, function(resp) {
            same(resp, -1, "responded with 'queued'");
            start();
        });
        same(alooma.test.persistence.props['__mpap'], [ { a: 2 } ], "queued append saved");
    });
    ...
    test("append hits server immediately if identified", 2, function() {
        ...
        stop();
        s = alooma.test.people.append({ a: 3 }, function(resp) {
            same(resp, 1, "responded with 'success'");
            start();
        });
...
mpmodule("alooma.people.union");
    ...
    test("union calls provided callback", 2, function() {
        ...
        alooma.test.people.union({'key': 'val'}, function(resp) {
    ...
    test("union queues and merges data", 4, function() {
        ...
        alooma.test.people.union(_union1, function(resp) {
            same(resp, -1, "responded with 'queued'");
        });
        same(alooma.test.persistence.props['__mpu'], { 'key1': ['val1.1'], 'key2': ['val1.2']}, "queued union saved");
        alooma.test.people.union(_union2, function(resp) {
            same(resp, -1, "responded with 'queued'");
        });
        same(alooma.test.persistence.props['__mpu'], { 'key1': ['val1.1', 'val2.1'], 'key2': ['val1.2'], 'key3':['val2.3'] }, "queued union saved");
    });
    ...
    test("set after union clobbers union queue", 4, function() {
        ...
        alooma.test.people.union(_union, function(resp) {
            same(resp, -1, "responded with 'queued'");
        });

        alooma.test.people.set(_set, function(resp) {
            same(resp, -1, "responded with 'queued'");
        });

        same(alooma.test.persistence.props['__mpu'], {'key2': ['val2']}, "set after union empties union queue");
        ok(contains_obj(alooma.test.persistence.props['__mps'], {'key1': 'set_val'}), "set after union enqueues set");
    });
    ...
    test("union sends immediately if identified", 2, function() {
        ...
        stop();
        s = alooma.test.people.union({ a: [3] }, function(resp) {
            same(resp, 1, "responded with 'success'");
            start();
        });
...
mpmodule("alooma.people.track_charge");
    ...
    test("track_charge accepts properties", 1, function() {
        ...
        var i = alooma.people.track_charge(amt, { '$time': time });
        ok(contains_obj(i['$append']['$transactions'], charge), '.track_charge() correctly appends to the $transactions object');
    });
...
mpmodule("alooma.people flushing");
    ...
    test("identify with no params", 2, function() {
        ...
        alooma.test.identify();
        if (window.console) {
            console.error = old_error;
        }
        same(alooma.test.get_distinct_id(), distinct_id);
    ...
    test("identify flushes set queue", 4, function() {
        alooma.test.people.set("a", "b");
        alooma.test.people.set("b", "c");

        stop();
        alooma.test.identify(this.id, function(resp, data) {
            ok(resp == 1, "Successful write");
            ok(contains_obj(data["$set"], { "a": "b", "b": "c" }));
            same(alooma.test.persistence.props['__mps'], {}, "Queue is cleared after flushing");
            // reload persistence to make sure it's persisted correctly
            alooma.test.persistence.load();
            same(alooma.test.persistence.props['__mps'], {}, "Empty queue is persisted");
            start();
        });
    });
    ...
    test("identify no params flushes set queue", 4, function() {
        ...
        stop();
        alooma.test.identify(undefined, function(resp, data) {
            ok(resp == 1, "Successful write");
            ok(contains_obj(data["$set"], { "a": "b", "b": "c" }));
            same(alooma.test.persistence.props['__mps'], {}, "Queue is cleared after flushing");
            // reload persistence to make sure it's persisted correctly
    ...
    test("identify flushes set_once queue", 4, function() {
        alooma.test.people.set_once("a", "b");
        alooma.test.people.set_once("b", "c");

        stop();
        alooma.test.identify(this.id, function() {}, function() {}, function() {}, function(resp, data) {
            ok(resp == 1, "Successful write");
            ok(contains_obj(data["$set_once"], { "a": "b", "b": "c" }));
            same(alooma.test.persistence.props['__mpso'], {}, "Queue is cleared after flushing");
            // reload persistence to make sure it's persisted correctly
            alooma.test.persistence.load();
            same(alooma.test.persistence.props['__mpso'], {}, "Empty queue is persisted");
            start();
        });
    });
    ...
    test("identify flushes add queue", 4, function() {
        var _this = this;
        alooma.test.people.increment("a");
        alooma.test.people.increment("b", 2);

        stop();
        alooma.test.identify(this.id, function() {}, function(resp, data) {
            ok(resp == 1, "Successful write");
            same(data, { "$token": _this.token, "$distinct_id": _this.id, "$add": { "a": 1, "b": 2 } });
            same(alooma.test.persistence.props['__mpa'], {}, "Queue is cleared after flushing");
            // reload persistence to make sure it's persisted correctly
            alooma.test.persistence.load();
            same(alooma.test.persistence.props['__mpa'], {}, "Empty queue is persisted");
            start();
        });
    });
    ...
    test("identify flushes append queue", 12, function() {
        ...
        alooma.test.people.append("a", 2);
        alooma.test.people.append({ 'b': 'asdf' });
        alooma.test.people.append("c", [1,2,3]);

        same(alooma.test.persistence.props[queue_name].length, 3, 'Queue has 3 elements before flushing');

        stop(); stop(); stop();
        alooma.test.identify(this.id, function() {}, function() {}, function(resp, data) {
            ok(resp == 1, "Successful write");
            ...
            run++;
            if (run == 3) {
                same(alooma.test.persistence.props[queue_name], [], "Queue is cleared after flushing");
                // reload persistence to make sure it's persisted correctly
                alooma.test.persistence.load();
                same(alooma.test.persistence.props[queue_name], [], "Empty queue is persisted");
            };
            start();
        });
    });
    ...
    test("identify flushes union queue", 4, function() {
        ...
        alooma.test.people.union(_union1);
        alooma.test.people.union("key2", ["val2.2"]);

        stop();
        var noop = function() {};
        alooma.test.identify(this.id, noop, noop, noop, noop, function(resp, data) {
            same(resp, 1, "Successful write");
            same(data["$union"], {'key1': ['val1.1'], 'key2': ['val1.2', 'val2.2']});
            same(alooma.test.persistence.props['__mpu'], {}, 'Queue is cleared after flushing');

            alooma.test.persistence.load();
            same(alooma.test.persistence.props['__mpu'], {}, 'Empty queue is persisted');
            start();
        });
    });
    ...
    test("identify clears out queues before server response", 8, function() {
        alooma.test.people.set("key", "val");
        alooma.test.people.increment("num");
        alooma.test.people.append("ary", 'val');
        alooma.test.people.union("stuff", 'val');

        alooma.test.identify(this.id);

        same(alooma.test.persistence.props['__mpap'], []);
        same(alooma.test.persistence.props['__mpu'], {});
        same(alooma.test.persistence.props['__mpa'], {});
        same(alooma.test.persistence.props['__mps'], {});
        // reload persistence to make sure it's persisted correctly
        alooma.test.persistence.load();
        same(alooma.test.persistence.props['__mpap'], []);
        same(alooma.test.persistence.props['__mpu'], {});
...
mpmodule("alooma.people.delete_user");

    test("delete_user", 2, function() {
        d = alooma.test.people.delete_user();
...
mpmodule("in-app notification display");
    ...
    test("calling _show_notification with bad data does not halt execution", 1, function() {
        alooma.test._show_notification();
        alooma.test._show_notification(15);
        alooma.test._show_notification('hi');
        alooma.test._show_notification({body: null});
        alooma.test._show_notification({bla: 'bla'});
        ok(true);
    });
...
mpmodule("verbose output");
    ...
    asyncTest("track endpoint returns json when verbose=1", 1, function() {
        alooma.test.set_config({ verbose: true });
    ...
    asyncTest("engage endpoint returns json when verbose=1", 1, function() {
        ...
        alooma.test.identify('bilbo');
        alooma.test.people.set('test', 123, function(response) {
...
mpmodule("debug helpers");
    ...
    test("toString", 2, function() {
        same(alooma.test.toString(), "alooma.test");
        same(alooma.test.people.toString(), "alooma.test.people");
    });
...
if( /Android|iPhone|iPad|iPod|BlackBerry|Windows Phone/i.test(navigator.userAgent) ) {
    mpmodule("mobile tests");
        test("device property included", 1, function() {
            stop();
            alooma.test.track("test_device", {}, function(r, data) {
